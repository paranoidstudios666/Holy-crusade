...
public class Loots2Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (Math.random() < 0.5) {
			...
			if (Math.random() < 0.2) {
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = HolyCrusadeModEntities.BONES.get().spawn(_level, BlockPos.containing(x + 0.5, y, z + 0.5), EntitySpawnReason.MOB_SUMMONED);
